...
#  complete turnover by currency
  def turnover_comp(self):
    df_turnover_comp = self.df_turnover[self.df_turnover['В***'].isin(self.curr_name)]
    turnover_curr = df_turnover_comp['С***'].sum()
    return turnover_curr

# refund turnover by currency
  def turnover_refund(self):
    df_turnover_ref = self.df_turnover_refund[self.df_turnover_refund['В***'].isin(self.curr_name)]
    turnover_ref_curr = df_turnover_ref['С***'].sum()  # оборот рефандов Евро
    return turnover_ref_curr


  def turnover_comp_EU(self):  # Turnover for EU countries
    df_turnover_comp = self.df_turnover[(self.df_turnover['В***'].isin(self.curr_name)) & (self.df_turnover['Н***'].isin(self.EU))]
    turnover_curr_eu = df_turnover_comp['С***'].sum()
    return turnover_curr_eu

  def turnover_comp_NonEU(self):
    df_turnover_comp = self.df_turnover[(self.df_turnover['В***'].isin(self.curr_name)) & (~self.df_turnover['Н***'].isin(self.EU))]
    turnover_curr_non_eu = df_turnover_comp['С***'].sum()
    return turnover_curr_non_eu

  def turnover_refund_EU(self):
    df_turnover_ref = self.df_turnover_refund[(self.df_turnover_refund['В***'].isin(self.curr_name)) & (self.df_turnover_refund['Н***'].isin(self.EU))]
    turnover_ref_curr_EU = df_turnover_ref['С***'].sum()  # оборот рефандов Евро
    return turnover_ref_curr_EU

  def turnover_refund_NonEU(self):
    df_turnover_ref = self.df_turnover_refund[(self.df_turnover_refund['В***'].isin(self.curr_name)) & (~
      self.df_turnover_refund['Н***'].isin(self.EU))]
    turnover_ref_curr_NonEU = df_turnover_ref['С***'].sum()  # оборот рефандов Евро
...
#  refund count by currency
  ...
  def count_refund(self):
    ...
    refund_count_df = self.df_turnover_refund[self.df_turnover_refund['В***'].isin(self.curr_name)]
    refund_count_curr = refund_count_df['С***'].count()  # кол-во рефандов by currency
    return refund_count_curr

  # Quantity of transactions
  def count_complete(self):
    return self.df_turnover['С***'].count()
...
def wb_holdback(statement_sheet, date_from_obj, date_to_obj, now_date,time):
  ...
  for i in range(***, ***):                                         # hold size in statement
  ...
  hold_sheet = hbook['WB']
  for i in range(***, ***):  # search an empty raw
    value = hold_sheet.cell(row=i, column=1).value
    if value is None:
      hold_sheet.cell(row=i, column=1).value = '***'
  ...
  for curr in hold_dict:
    for j in range(***, ***):                             # currency size in hold file
  ...
  for m in range(***, ***):  # search the holdback rerturn date
    ...
    if value1 > date_to_obj and value <= date_to_obj:
      ...
      for n in range(***, ***):                            # currency size in hold file
  ...
  for curr in holdreturn_dict:
    for i in range(***, ***):           #holdback return range in Excel
